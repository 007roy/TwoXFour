...
void ServerPlayerSpawnerComponent::Reflect(AZ::ReflectContext* reflection)
{
    if (auto serializationContext = azrtti_cast<AZ::SerializeContext*>(reflection))
    {
        serializationContext->Class<ServerPlayerSpawnerComponent, Component>()
            ->Version(1);

        if (auto editContext = serializationContext->GetEditContext())
        {
            editContext->Class<ServerPlayerSpawnerComponent>("ServerPlayerSpawnerComponent", "Server Player Spawner Component")
...
void ServerPlayerSpawnerComponent::OnMemberJoined(GridMate::GridSession* session, GridMate::GridMember* member)
{
...
void ServerPlayerSpawnerComponent::OnSliceInstantiated(const AZ::Data::AssetId&, const SliceComponent::SliceInstanceAddress& address)
{
    ...
    if (iter != m_joiningPlayers.end())
    {
        ...
        for (Entity* entity : address.second->GetInstantiated()->m_entities)
        {
            ServerPlayerBodyBus::Event(entity->GetId(), &ServerPlayerBodyBus::Events::SetAssociatedPlayerId, playerId);
        }
    }
    m_joiningPlayers.erase(iter);
}

void ServerPlayerSpawnerComponent::SpawnPlayer(GridMate::MemberIDCompact playerId)
{
    const Transform t = Transform::CreateTranslation(Vector3::CreateAxisY(10.f - m_playerCount * 1.f));
    m_playerCount++;

    AZ_Printf("SAM", "Spawning player @ %f %f %f",
    ...
    SpawnerComponentRequestBus::EventResult(tckt, GetEntityId(), &SpawnerComponentRequestBus::Events::SpawnRelative, t);
