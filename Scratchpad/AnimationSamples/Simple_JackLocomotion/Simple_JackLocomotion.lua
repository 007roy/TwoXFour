...
function EmotionFXTests:OnActivate()
    ...
    self.localUserId = LocalUserRequestBus.Broadcast.GetLocalUserIdAssignedToLocalPlayerSlot(0);
    -- Achievement IDs for achievements unlocked through this script. IDs are created through the dev
    -- portals/metadata for supported platforms and should be designed such that they are consistent between platforms
    self.aimAchId = "3";
    --Set up the achievement tracking variables and bus so we can reguest their data and unlock where appropriate
    self.aimAchEarned = false;
    self.achNotHandler = AchievementNotificationBus.CreateHandler(self);
    self.achNotHandler:Connect();
    
    local qParams = QueryAchievementParams();
    qParams.localUserId = self.localUserId;
    qParams.achievementId = self.aimAchId;
    AchievementRequestBus.Broadcast.QueryAchievementDetails(qParams);
    
    self.presenceNotHandler = PresenceNotificationBus.CreateHandler(self);
    local setPresenceParams = SetPresenceParams();
    setPresenceParams.localUserId = self.localUserId;
    setPresenceParams.presenceToken = "PRESENCE_SIMPLE_JACK_LOCOMOTION";
    setPresenceParams.presenceString = "Locomoting Jack";
    setPresenceParams.languageCode = "en";
    PresenceRequestBus.Broadcast.SetPresence(setPresenceParams);
end

function EmotionFXTests:OnAchievementDetailsQueried(id, details)
    if (details.id == tonumber(self.aimAchId) and details.unlocked == true) then
        self.aimAchEarned = true;
    end
end
...
function EmotionFXTests:HandleInputValue(floatValue)
    ...
    elseif (InputEventNotificationBus.GetCurrentBusId() == self.aimInputBusId) then
        ...
        if (self.aimAchEarned == false) then
            local uParams = UnlockAchievementParams();
            uParams.achievementId = self.aimAchId;
            uParams.percentage = 100;
            uParams.localUserId = self.localUserId;
            AchievementRequestBus.Broadcast.UnlockAchievement(uParams);
            self.aimAchEarned = true;
        end
    end
end
...
function EmotionFXTests:OnDeactivate()
    ...
    self.achNotHandler:Disconnect();
    self.presenceNotHandler:Disconnect();
end
